export default {
  methods: {
    async criticaItem(item, args, self, atualiza) {
      ...
      if (item.vlComBDI * item.qtd == 0) erro.push({
        'erro': 'Item com Preço/Qtde zerada'
      })
      //###### varifica item cadastrado #####
      //###### varifica item cadastrado #####
      if (cadastrado == '' || cadastrado == undefined) {
        erro.push({
          'erro': 'Item não cadastrado nesse Orgão/Base/UF'
        })
        return JSON.stringify(erro) + '||' + JSON.stringify(alerta)
      }
      //###### varifica descrição do item cadastrado #####
      if (cadastrado.descr.toUpperCase() !== item.descr.toUpperCase()) alerta.push({
        'alerta': 'Verifique a descrição do item'
      })
      ...
      if (!atualiza) {
        ...
        if (repetido.length > 1) {
          ...
          if (numrep.length > 0) erro.push({
            'erro': "Item repetido com valor diferente."
          })
        }
      }
      ...
      if (desonerado == "1") {
        ...
        refer = parseFloat(cadastrado.precodeson * 1)
      } else {
        valor = parseFloat(cadastrado.precooner * (1 + (BDI / 100))).toFixed(2)
        refer = parseFloat(cadastrado.precooner * 1)
      }
      if (parseFloat(item.vlComBDI) > parseFloat(valor) && valor > 0) {
        erro.push({
          'erro': 'Item com preço maior que a referência'
        })
      }
      if (parseFloat(item.vlComBDI) < parseFloat(valor * 0.7) && valor > 0) {
        alerta.push({
          'alerta': 'Item com preço menor que a 70% da referência'
        })
      }
      // #####  retorno da crítica #####
      return JSON.stringify(erro) + '||' + JSON.stringify(alerta) + '||' + refer
    }
  },
}